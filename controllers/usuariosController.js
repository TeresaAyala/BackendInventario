...
exports.createUsuario = async (req, res) => {
  try {
    const { nombre, apellido, email, telefono, password, roles, status } = req.body;
    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);

    const newUsuario = new Usuario({ nombre, apellido, email, telefono, roles, status, password: hashedPassword });
...
exports.login = async (req, res) => {
    try {
        ...
        const token = jwt.sign(
            { id: usuario._id, roles: usuario.roles, email: usuario.email },
            process.env.JWT_SECRET || 'secret_key',
            { expiresIn: '8h' }
        );
